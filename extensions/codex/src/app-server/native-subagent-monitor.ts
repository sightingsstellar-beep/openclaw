/**
 * Mirrors Codex native subagent lifecycle into silent OpenClaw task telemetry.
 * Native Codex remains the sole completion-delivery path to the parent.
 */
...
type ParentState = {
  ...
  // Overlapping turns share one parent-scoped telemetry mirror.
  ownerCount: number;
...
type ChildState = {
  ...
  awaitingNativeEnvelope: boolean;
  terminal: boolean;
...
const DEFAULT_NATIVE_ENVELOPE_WAIT_MS = 60_000;
...
class Monitor {
  ...
  private readonly nativeEnvelopeWaitMs: number;
  ...
  constructor(
    ...
    this.nativeEnvelopeWaitMs = Math.max(
      1,
      options.nativeEnvelopeWaitMs ?? DEFAULT_NATIVE_ENVELOPE_WAIT_MS,
    );
  ...
  registerParent(params: {
    ...
    let registered = true;
    return {
      hasPendingChildren: () => this.hasPendingChildren(parentThreadId),
      unregister: () => {
        if (!registered) {
          return;
        }
  ...
  private hasPendingChildren(parentThreadId: string): boolean {
    return [...this.childStates.values()].some(
      (childState) =>
  ...
  private observeActiveChild(childState: ChildState): void {
    childState.awaitingNativeEnvelope = false;
  ...
  private async handleChildTurnCompletion(notification: CodexServerNotification): Promise<void> {
    ...
    this.observeTerminalCompletion(childState);
  }

  /** Reads one child through app-server history for telemetry only. */
  ...
  private observeTerminalCompletion(childState: ChildState): void {
    if (childState.terminal || childState.awaitingNativeEnvelope) {
      return;
    }
    childState.awaitingNativeEnvelope = true;
    childState.fallbackCompletion = undefined;
    this.clearRecoveryTimers(childState);
    // The child-side terminal event precedes the trusted parent-scoped native
    // completion envelope. Keep a retired shared app-server alive long enough
    // for that sole authoritative handoff, then fail closed without creating a
    // recovery model turn if the envelope never arrives.
    childState.recoveryTimer = setTimeout(() => {
      ...
      if (
        ...
        childState.terminal ||
        !childState.awaitingNativeEnvelope
      ) {
        return;
      }
      childState.awaitingNativeEnvelope = false;
      this.unregisterChild(childState);
    }, this.nativeEnvelopeWaitMs);
    unrefTimer(childState.recoveryTimer);
  }

  private settleNativeParentCompletion(
    state: ParentState,
    childState: ChildState,
    ...
    childState.recoveryTimer = setTimeout(() => {
      ...
      void this.reconcileChildThread(childState.childThreadId)
        ...
        .then(async (reconciled) => {
          ...
          // Give thread/read two persistence windows before settling telemetry;
          // a just-written final may not yet be visible in history.
          if (fallback && childState.recoveryAttempt >= 2) {
            this.observeTerminalCompletion(childState);
            return;
          }
          this.scheduleRecoveryPoll(childState);
